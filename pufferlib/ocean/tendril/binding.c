...
void c_step(Tendril* env) {
    ...
    if (env->target_state == TARGET_ACTIVE) {
        for (int i = 0; i < NUM_JOINTS; i++) {
            ...
            // MG996R servo: max 60°/sec, at 50Hz = 1.2°/step max
            float max_delta_per_step = (SERVO_SPEED_DEG_SEC * TAU) * (M_PI / 180.0f);
            float requested_delta = -env->actions[i] * max_delta_per_step; // Sign flip for correct learning
